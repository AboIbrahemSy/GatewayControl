...
function RuntimePill({ status, t }: { status: RuntimeServiceStatus; t: Translate }) { return <span className="shrink-0 rounded-full bg-stone-500/10 px-2.5 py-1 text-[0.65rem] font-extrabold">{t(status === 'healthy' ? 'runtimeHealthy' : status)}</span> }
...
function friendlyError(error: unknown, t: Translate) { if (error instanceof ApiError) { if (error.code === 'project_protected' || error.code === 'protected_logs_owner_required') return t('protectedRuntime'); if (error.code === 'telemetry_stale') return t('staleRuntime'); if (error.code === 'agent_unavailable') return t('offlineRuntime'); if (error.code === 'operation_active') return t('runtimeOperationActive'); if (error.status === 403) return t('forbidden'); if (error.status === 400) return t('validationError') } return t('requestFailed') }
