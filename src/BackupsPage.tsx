import { Archive, CheckCircle2, DatabaseBackup, HardDrive, KeyRound, Network, RefreshCw, RotateCcw, Server, ShieldCheck, TriangleAlert } from 'lucide-react'
...
export function BackupsPage({ t, locale, role }: { t: Translate; locale: Locale; role: Role }) {
  ...
  function resetSystemModal() { setPassphrase(''); setPassphraseConfirmation(''); setSystemModalError('') }
  ...
  async function stageSystemRestore(event: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(t('systemRestoreStaged')); setSystemRestoreBackup(null); resetSystemModal()
    } catch (caught) { setSystemModalError(systemRecoveryError(caught, t)) }
    finally { setBusy('') }
  }
  ...
  return <div className="mx-auto flex w-full max-w-[100rem] flex-col gap-6">
    ...
    {role === 'owner' && <Modal open={systemCreateOpen} title={t('createSystemBackup')} description={t('systemRecoveryScopeDescription')} closeLabel={t('cancel')} busy={busy === 'system-create'} maxWidthClass="max-w-xl" onClose={() => { setSystemCreateOpen(false); setSystemTarget('local'); resetSystemModal() }}><form onSubmit={createSystemBackup} className="flex flex-col gap-5 p-5 sm:p-6">{systemModalError && <Alert>{systemModalError}</Alert>}<div><p className="pb-2 text-xs font-extrabold">{t('backupTarget')}</p><div role="radiogroup" aria-label={t('backupTargetLabel')} className="grid grid-cols-2 gap-2"><Target active={systemTarget === 'local'} icon={HardDrive} label={t('localTarget')} onClick={() => setSystemTarget('local')} /><Target active={systemTarget === 'nas'} icon={Network} label={t('nasTarget')} onClick={() => setSystemTarget('nas')} /></div></div><Field label={t('systemPassphrase')}><input type="password" required minLength={16} maxLength={1024} autoComplete="new-password" value={passphrase} onChange={(event) => setPassphrase(event.target.value)} className={inputClass} /></Field><Field label={t('systemPassphraseConfirmation')}><input type="password" required minLength={16} maxLength={1024} autoComplete="new-password" value={passphraseConfirmation} onChange={(event) => setPassphraseConfirmation(event.target.value)} className={inputClass} /></Field><p className="text-xs font-semibold leading-5 text-stone-500 dark:text-stone-400">{t('systemPassphraseNotice')}</p><button disabled={busy !== ''} className={primaryButton}><KeyRound size={16} />{t('createSystemBackup')}</button></form></Modal>}
    {role === 'owner' && <Modal open={systemRestoreBackup !== null} title={t('stageSystemRestore')} description={t('systemRecoveryScopeDescription')} closeLabel={t('cancel')} busy={busy === 'system-restore'} maxWidthClass="max-w-xl" onClose={() => { setSystemRestoreBackup(null); resetSystemModal() }}><form onSubmit={stageSystemRestore} className="flex flex-col gap-5 p-5 sm:p-6">{systemModalError && <Alert>{systemModalError}</Alert>}<Notice>{t('systemRestoreDestructiveWarning')}</Notice><bdi dir="ltr" className="break-all rounded-xl bg-stone-100 p-3 font-mono text-xs dark:bg-white/[0.04]">#{systemRestoreBackup?.id}</bdi><Field label={t('systemPassphrase')}><input type="password" required minLength={16} maxLength={1024} autoComplete="new-password" value={passphrase} onChange={(event) => setPassphrase(event.target.value)} className={inputClass} /></Field><button disabled={busy !== ''} className={primaryButton}><RotateCcw size={16} />{t('confirmStageSystemRestore')}</button></form></Modal>}
  </div>
}
