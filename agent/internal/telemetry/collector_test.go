...
func TestCollectorReadsProcAndUsesFixedDockerFormat(t *testing.T) {
	...
	if snapshot.Node.MemoryTotalBytes != 1000*1024 || len(snapshot.Services) != 2 || snapshot.Services[0].Status != "healthy" || snapshot.Services[1].Status != "completed" {
...
func TestCollectorAggregatesReplicasByExactProjectAndServiceIdentity(t *testing.T) {
	...
	if services[0].Name != "alpha_beta/web" || services[0].Total != 2 || services[0].Running != 1 || services[0].Healthy != 1 || services[0].Completed != 1 || services[0].Status != "unknown" {
